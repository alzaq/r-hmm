...
### 9. Prechodova matice
Difinuji si matici přechodu mezi stavy HLAS a TICHO.
Jaká je pravděpodobnost přechodu ze stavu do stavu
1 TICHO
2 HLAS
...
for (i in 1:length(files)) {
  ...
  for (i in 1:(length(poleIndexu) - 1)) {
    ...
    pocetPrechodu <- pocetPrechodu + 1
  }
}

maticePrechodu <- maticePrechodu/pocetPrechodu
print(maticePrechodu)
```

### Rozdělení zvukových částí do clusterů
...
clusters <- kmeans(output, centers = pocetClusteru, iter.max = 1000)
...
maticePravdepodobnostiVystupu <- maticePravdepodobnostiVystupu / castIndex
...
### 9. viterbi 
```{r}
viter <- viterbi(hmm, c(5,5,9,9,31,1,1,1,58,1)) #clusters$cluster[1:80]
print(viter)

# vykreslit grafik
stopa <- as.integer(viter)
print(stopa)
plot(stopa, type="o", col="blue")
```
